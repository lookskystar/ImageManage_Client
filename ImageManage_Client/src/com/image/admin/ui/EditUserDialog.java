...
public class EditUserDialog extends JDialog {
	...
	public EditUserDialog(DictUsers user){
		this.user=user;
		comboboxInit();
		init();
		panelInit();
				...
				if ("".equals(py)) {
					...
					for (DictRoles listRole : usersService.listDictRoles()) {
						if (RoleComboBox.getSelectedItem().equals(
							...
							user.setRoleId(listRole.getRoleId());
						}
					}
					for (DictTeams listTeam : usersService.listDictTeams(sessionUser.getAreaId())) {
						if (BzComboBox.getSelectedItem().equals(
								listTeam.getTeamName())) {
							user.setTeamId(listTeam.getTeamId());
						}
					}
					user.setName(name);
					...
					user.setPy(py);
					user.setIdnum(txtIDK.getText().trim());
					user.setAreaId(sessionUser.getAreaId());
					user.setIsuser(1);
	...
	public void panelInit() {
		...
		btnEdit.setBounds(180, 200, 70, 22);
