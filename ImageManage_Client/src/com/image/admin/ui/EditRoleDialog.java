...
public class EditRoleDialog extends JDialog {
	...
	public EditRoleDialog(DictRoles role){
		this.role=role;
		init();
		panelInit();
		buttonInit();
	...
	public void init(){
		...
		roleLevel.add(btnCj);
		roleLevel.add(btnD);
		btnEdit = new JButton(ResourceUtils.getResourceByKey("JButton.edit_label"));
		CommonComponentUtils.setButtonStyle(btnEdit,70,24,null);
		...
		txtRolePy.setText(role.getRolePy());
		if(role.getRoleLevel()==0){
			btnCj.setSelected(true);
		}else{
			btnD.setSelected(true);
		}
		txtRoleNote.setText(role.getRoleNote());
	}
	/**
	 * 按钮控件初始化
	 */
		...
		btnEdit.addActionListener(new ActionListener() {
			...
			private void btnEdit_actionPerformed(ActionEvent e) {
				...
				if("".equals(roleName)){
					...
					DictRoles oldRole=rolesService.getDictRolesByName(roleName);
					if(oldRole!=null&&!roleName.equals(role.getRoleName())){
						JOptionPane.showMessageDialog(null, "修改失败,角色名称已经存在","错误",JOptionPane.ERROR_MESSAGE);
						txtRoleName.setText(role.getRoleName());
						txtRoleName.grabFocus();
					}else{
						role.setRoleName(roleName);
						role.setRolePy(txtRolePy.getText().trim());
						if (btnCj.isSelected()) {
							role.setRoleLevel(0);
						} else {
							role.setRoleLevel(1);
						}
						role.setRoleNote(txtRoleNote.getText().trim());
						rolesService.updateRole(role);
						JOptionPane.showMessageDialog(null, "修改成功", "信息",
								JOptionPane.INFORMATION_MESSAGE);
						EditRoleDialog.this.dispose();
					}
	...
	public void panelInit() {
		...
		btnEdit.setBounds(60, 230, 70, 22);
