...
import javax.swing.JOptionPane;
 ...
 public class AutoUpdateClient{
     ...
     public void update()
     {
         ...
         try
         {
             ...
             while(true)
             {
                 ...
                 if(len!=1)
                 {
                	 dialog.closeAndTip("读取标识位失败", JOptionPane.WARNING_MESSAGE);
                 ...
                 if(flag[0]==AUPD.CMD_DATA_SECT)//命令行
                 {
                     ...
                     if(len!=8)
                     {
                    	 dialog.closeAndTip("读取命令失败", JOptionPane.WARNING_MESSAGE);
                         Config.print("读取命令失败");
                         socketOut.write(Config.getCmd(AUPD.BYE));//结束
                         break;
                     }
                    ...
                    {
                    	dialog.closeAndTip("已经是最新版本，无需更新！", JOptionPane.WARNING_MESSAGE);
                ...
                }else if(flag[0]==AUPD.MARK_DATA_SECT || flag[0]==AUPD.MARK_DATA_END)//数据内容
                {
                    if(Config.parseCmd(cmd).equals(AUPD.SEND_FILE_ABSOULT))//接收文件全路径
                     {
                        ...
                        {
                        	dialog.setStatus("接收文件全路径‘"+currFileAbs+"’成功");
                            socketOut.write(Config.getCmd(AUPD.RECEIVED_FILE_ABSOULT));
                            Config.print("接收文件全路径‘"+currFileAbs+"’成功");
                        }else
                        {
                        	dialog.closeAndTip("接收文件全路径失败",JOptionPane.WARNING_MESSAGE);
                    ...
                    {
                    	dialog.closeAndTip("升级器异常!",JOptionPane.WARNING_MESSAGE);
                ...
                {
                	dialog.closeAndTip("升级异常!",JOptionPane.WARNING_MESSAGE);
    ...
    private boolean receiveFile(byte flag)
   {
    ...
    {
        AutoUpdateClient client = new AutoUpdateClient(new ProgressBarDialog(null,true));
        client.update();
    }
}