...
public class FileInfoTableModel extends AbstractTableModel {
	...
	protected boolean acceptFile(File file) {
		...
		if (file.isDirectory() && FTPUtils.getResourceByKeyToBoolean("isDirectorySelectionEnabled")) {
			accept = true;
		} else if (file.isFile() && FTPUtils.getResourceByKeyToBoolean("isFileSelectionEnabled")) {
			if (FTPUtils.useFileExtension()) {
				...
				if (index > 0) {
					...
					accept = FTPUtils.getFileNameExtension().contains(suffix);
					accept = accept && file.length() < FTPUtils.getResourceByKeyToLong("maxFileSize"); // 判断文件是否超过最大限制
				}
			} else {
				accept = true;
			}
