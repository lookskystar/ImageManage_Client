...
import com.image.util.FileCounter;
import com.image.util.MulFileUtil;
...
public class ReportTable {
	...
	public ReportTable(){
		dialog=new JDialog();
		dialog.setTitle("信息统计汇总");
		Container contentPanel=dialog.getContentPane();
		contentPanel.setLayout(new BorderLayout());
		contentPanel.add(createTablePanel(),BorderLayout.CENTER);
		contentPanel.add(createClosePanel(),BorderLayout.SOUTH);
		dialog.setMinimumSize(new Dimension(1200, 600));
		//初始化位置
...
//		frame.setVisible(true);
	...
	private JTable createTotalTable(Object[][] rows){
		JTable table=CommonComponentUtils.createTable(totalColumnNames, rows);
		return table;
	}
	...
	private JTable createDetailTable(Object[][] rows) {
		JTable jtable=CommonComponentUtils.createTable(detailColumnNames, rows);
...
//		//解析本地文件
		for (File file : fileList) {
			try{
				...
				if(file.getName().matches(".*\\.((?i)jpg)$") || file.getName().matches(".*\\.((?i)avi)$")){
					...
					json.put("area_id", FTPUtils.getResourceByKey("areaid"));
					...
					json.put("server_url", "/"+YMD_SDF.format(date)+"/"+tempArr[2].substring(0,5)+"/"+tempArr[2].substring(5)+"/"+FTPUtils.getResourceByKey("areaid")+"_"+tempArr[1]+"_"+HMS_SDF.format(date)+type);
		...
		PostMethod post = new PostMethod(FTPUtils.getResourceByKey("getDateUrl"));
