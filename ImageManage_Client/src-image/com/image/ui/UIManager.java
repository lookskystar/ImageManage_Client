...
import com.image.common.util.FTPUtils;
import com.image.common.util.ImageUtils;
import com.image.ftp.FTPFileUploadThread;
import com.image.util.AppletParam;
import com.image.util.UOperate;
...
//	private static final String SYNC_TEXT = "同步时间(T)";
	...
	private JPanel createControlPanel() {
		...
		autoBtn.setIcon(ImageUtils.createImageIcon("add.gif"));
		autoBtn.setUI(new
		...
		controlBtn.setForeground(Color.white);
		controlBtn.setIcon(ImageUtils.createImageIcon("add.gif"));
		controlBtn.setUI(new
        		BEButtonUI().setNormalColor(BEButtonUI.NormalColor.green));
...
//		syncBtn.addActionListener(this);
		...
		clearBtn.setIcon(ImageUtils.createImageIcon("recycle.gif"));
		clearBtn.setUI(new
        		BEButtonUI().setNormalColor(BEButtonUI.NormalColor.green));
		...
		countBtn.setIcon(ImageUtils.createImageIcon("save.gif"));
		countBtn.setUI(new
        		BEButtonUI().setNormalColor(BEButtonUI.NormalColor.green));
...
//		}else{
			...
			if (FTPUtils.useFileExtension()) {
				String extension = FTPUtils.getResourceByKey("fileNameExtension").trim();
	
				FileNameExtensionFilter filter = new FileNameExtensionFilter(
						"可上传的文件格式：" + extension, FTPUtils.getFileNameExtensionArray());
	
				chooser.setAcceptAllFileFilterUsed(FTPUtils.getResourceByKeyToBoolean("acceptAllFileFilterUsed"));
				chooser.setFileFilter(filter);
			}
			
			if (FTPUtils.getResourceByKeyToBoolean("isDirectorySelectionEnabled") && 
					FTPUtils.getResourceByKeyToBoolean("isFileSelectionEnabled")) {
				chooser.setFileSelectionMode(JFileChooser.FILES_AND_DIRECTORIES);
			} else if (FTPUtils.getResourceByKeyToBoolean("isDirectorySelectionEnabled")) {
				chooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
			} else {
				chooser.setFileSelectionMode(JFileChooser.FILES_ONLY);
			}
	
			chooser.setFileHidingEnabled(FTPUtils.getResourceByKeyToBoolean("isFileHidingEnabled"));
			chooser.setMultiSelectionEnabled(FTPUtils.getResourceByKeyToBoolean("isMultiSelectionEnabled"));
